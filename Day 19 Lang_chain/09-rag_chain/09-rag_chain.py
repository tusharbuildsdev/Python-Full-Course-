...
docs = [
    Document(
        page_content="Refunds are processed within 5 business days to the original payment method.",
        metadata={"source": "refunds"},
    ),
    Document(
        page_content="We ship across India; standard delivery takes 4 to 7 working days.",
        metadata={"source": "shipping"},
    ),
    Document(
        page_content="Cash on delivery is available for orders under 5000 rupees.",
        metadata={"source": "payment"},
    ),
    Document(
        page_content="Our office is in Pune; support is open 9am to 6pm, Monday to Saturday.",
        metadata={"source": "support"},
    ),
]
embeddings = HuggingFaceEmbeddings(model_name="all-MiniLM-L6-v2")
retriever = Chroma.from_documents(docs, embedding=embeddings).as_retriever(
    search_kwargs={"k": 2}
)
...
if not os.getenv("GROQ_API_KEY"):
    print(
        "No GROQ_API_KEY -- showing what the retriever feeds the prompt, then stopping.\n"
    )
    for q in questions:
        print(f"Q: {q}")
